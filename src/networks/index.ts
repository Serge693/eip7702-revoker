...
export const networks: Record<string, ReturnType<typeof defineChain>> = {
...
export function getNetworkByName(name: string): ReturnType<typeof defineChain> | null {
  const key = name.toLowerCase().trim();
  return networks[key] ?? networks[aliases[key]] ?? null;
}

export function getNetworks(): ReturnType<typeof defineChain>[] {
  return Object.values(networks);
}

export function getAllNetworkKeys(): string[] {
  return Object.keys(networks);
}

export function deduplicateNetworks(
  list: ReturnType<typeof defineChain>[],
): ReturnType<typeof defineChain>[] {
  return [...new Map(list.map((n) => [n.id, n])).values()];
}
